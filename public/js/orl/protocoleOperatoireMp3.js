...
function AppelLecteurMp3(id_admission){
	...
	var chemin = tabUrl[0]+'public/orl/afficher-mp3-protocole';
	$.ajax({
		url: chemin ,
		type: 'POST',
...
function scriptAjoutMp3(){
	...
	$(function () {
		$('#my_form').change(function (e) {
			...
			var chemin = tabUrl[0]+'public/orl/ajouter-mp3-protocole';
			$.ajax({
				url: chemin ,
				...
				success: function (response) {
					...
					$.ajax({
						url: tabUrl[0]+'public/orl/inserer-bd-mp3-protocole',
						type: $form.attr('method'),
...
function supprimerAudioMp3(id){ 
	...
	var id_admission = $("#id_admission").val();
	var chemin = tabUrl[0]+'public/orl/supprimer-mp3-protocole';
