...
class DemandeExamenMorphoPdf
{
	...
	public function addNotePlusieursExamensMorphologique(){
	...
	public function setEnTete(){
		...
		$this->_page->drawText('Demande d\'examen Morphologique',
	...
	protected function listeDesExamensMorpho(){
		$ExamensMorpho = array(
			
				'8' => 'Radiographie du CAVUM',
				'9' => 'Radiographie du THORAX',
				'10' => 'Echographie CERVICALE',
				'11' => 'TDM des Sinus',
				'12' => 'Audiogramme tonal',
		);
		return $ExamensMorpho;
	}
	
	
	protected  function getNoteMorpho(){
		...
		$this->_page->drawText('PRENOM & NOM ( Numéro Dossier) :',
		    $this->_leftMargin+85,
		    $this->_yPosition);
		...
		while($this->_yPosition > 70) {
			...
			if($this->_DonneesDemandeMorph && $i == 1){
				...
				$this->_page->drawLine(
				    $this->_leftMargin,
				    $this->_yPosition-5,
				    $this->_pageWidth - $this->_leftMargin - 425,
				    $this->_yPosition-5
				    );
 			...
 			if( $i <= ($examensMorpho) ){
 				...
 				if($this->_DonneesDemandeMorph[$i]){
 				    ...
 				    $this->_page->setFont($this->_newTimeGras, 25);
					$this->_page->drawText(iconv ('UTF-8' ,'ISO-8859-1' ,$this->listeDesExamensMorpho()[$this->_DonneesDemandeMorph[$i]]),
					    $this->_leftMargin,
					    $this->_yPosition-20);
	...
	public function AfficherExamenParPage(){
	    ...
	    while($this->_yPosition > 70) {
	        ...
	        if($this->_DonneesDemandeMorph){
	            
	            $this->_page->setFont($this->_newTimeGras, 30);
	            $this->_page->drawText(iconv ('UTF-8' ,'ISO-8859-1' ,$this->listeDesExamensMorpho()[$this->_DonneesDemandeMorph]),
	                $this->_leftMargin,
	                $this->_yPosition+350);
	            $this->_page->setFont($this->_policeContenu, 12);
	            $this->_page->drawText(iconv ('UTF-8' ,'ISO-8859-1' ,$this->_notesDemandeMorph),
	                $this->_leftMargin,
	                $this->_yPosition+250);
