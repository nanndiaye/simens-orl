...
class HospitalisationPdf
{
	...
	public function setEnTete(){
		...
		$this->_page->drawText('Demande d\'hospitalisation',
	...
	public function setDonneesPatientH($donneesPatient){
		$this->_DonneesPatient = $donneesPatient;
	}
	
	public function setDonneesMedecinH($donneesMedecin){
		$this->_DonneesMedecin = $donneesMedecin;
	}
	
	public function setDonneesDemandeH($donneesDemande){
		$this->_DonneesDemande = $donneesDemande;
	}
	
	public function setIdConsH($id_cons){
	...
	protected  function getNoteH(){
		...
		$tab = $this->scinderText($this->_DonneesDemande['motif_hospitalisation']);
		
		while($this->_yPosition > 70 ) {
			if($i==0){
				$this->_page->setFont($this->_newTimeGras, 14);
				$this->_page->drawText('Date fin prevue :   ',
						$this->_leftMargin,
						$this->_yPosition);
				$this->_page->setFont($this->_policeContenu, 14);
				$this->_page->drawText(iconv ('UTF-8' ,'ISO-8859-1' ,$this->_DonneesDemande['date_fin_hospitalisation_prevue']),
